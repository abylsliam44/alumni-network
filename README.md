...
## 📍 Порты и сервисы (у меня заняты остальные порты, поэтому вы тоже используйте эти пж!)
...
Используйте `.env.example` как шаблон; ключи (в т.ч. `GOOGLE_AI_API_KEY`) задаются там и пробрасываются в контейнеры.
